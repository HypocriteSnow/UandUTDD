...
namespace ArknightsLite.Editor.Tests.LevelEditor {
    public class WhiteboxGenerationServiceTests {
        ...
        [Test]
        public void BuildPreview_SpawnMarkerTile_UsesSemanticVisualTypeInsteadOfDefaultForbidden() {
            var workspace = LevelEditorWorkspace.CreateNew("Tutorial_01");
            workspace.AddSpawnMarker(new Vector2Int(1, 1));

            var result = WhiteboxGenerationService.BuildPreview(workspace);

            Assert.IsTrue(result.TryGetTile(1, 1, out var spawnTile));
            Assert.AreEqual(WhiteboxGenerationService.WhiteboxVisualType.Spawn, spawnTile.VisualType);
            Assert.AreEqual("R1", spawnTile.SemanticLabel);

            Assert.IsTrue(result.TryGetTile(0, 0, out var defaultTile));
            Assert.AreEqual(WhiteboxGenerationService.WhiteboxVisualType.Forbidden, defaultTile.VisualType);
        }
        ...
        public void GenerateIntoOpenScene_NewWorkspaceDoesNotRenderImplicitLegacySpawnOrGoal() {
            ...
            try {
                ...
                Assert.IsTrue(string.IsNullOrWhiteSpace(originTile.SpawnMarkerId));
                Assert.IsTrue(string.IsNullOrWhiteSpace(cornerTile.GoalMarkerId));
            } finally {
                if (root != null) {
                    Object.DestroyImmediate(root.gameObject);
                }
        ...
        public static void RunFromCommandLine() {
            ...
            new WhiteboxGenerationServiceTests().BuildPreview_SpawnMarkerTile_UsesSemanticVisualTypeInsteadOfDefaultForbidden();
